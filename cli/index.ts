...
export async function main(argv: string[], io: Pick<Console, 'log' | 'error'> = console): Promise<number> {
		...
		case 'mount': {
			try {
				...
				if (result.url) {
					...
					io.log(
						`weave: ${result.conceptCount} concepts in ${Math.round(result.buildMs)}ms`,
					);
